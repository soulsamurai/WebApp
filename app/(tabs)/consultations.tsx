...
export default function ConsultationsScreen() {
  ...
  const canAddConsultations = user?.role === 'teacher' || user?.role === 'admin';

  const handleGoogleSheetsIntegration = () => {
    console.log('Google Sheets integration for consultations');
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Консультации</Text>
        {canAddConsultations && (
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        {canAddConsultations && isAddFormExpanded && (
          <View style={styles.addFormContainer}>
            <View style={styles.addForm}>
              <Text style={styles.addFormTitle}>Быстрое добавление консультации</Text>
              <Text style={styles.addFormDescription}>
                Создайте новую консультацию для студентов
              </Text>
              <TouchableOpacity 
        ...
        <View style={styles.consultationsList}>
          {consultations.length === 0 ? (
            <View style={styles.emptyState}>
              <Text style={styles.emptyText}>Нет запланированных консультаций</Text>
              {canAddConsultations && (
                <Text style={styles.emptySubtext}>
                  Нажмите "Добавить" чтобы создать новую консультацию
                </Text>
              )}
            </View>
          ) : (
